...
#define AW882XX_DRIVER_VERSION "v1.10.0.16"
...
static int aw882xx_force_boost_8v_mode_info(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_info *uinfo)
{
	int count;
...
static int aw882xx_force_boost_8v_mode_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.kcontrol_codec(kcontrol);
	...
	ucontrol->value.integer.value[0] = aw_dev->force_boost_8v_mode;

	aw_dev_info(aw882xx->dev, "get force boost 8v mode: %ld",
				ucontrol->value.integer.value[0]);
	return 0;
}

static int aw882xx_force_boost_8v_mode_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int ret = -EINVAL;
	aw_snd_soc_codec_t *codec =
	...
	mutex_lock(&aw882xx->lock);
	ret = aw_dev->ops.aw_set_force_boost_8v(aw_dev, ctrl_value);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "set force boost 8v reg failed,ret = %d", ret);
		ctrl_value = AW_FORCE_BOOST_8V_DISABLE;
	}

	aw_dev->force_boost_8v_mode = ctrl_value;
	aw_dev_info(aw882xx->dev, "set force boost 8v mode: %d", ctrl_value);
	mutex_unlock(&aw882xx->lock);
...
static const DECLARE_TLV_DB_SCALE(digital_gain, 0, 10, 0);
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	value = (uint32_t)ucontrol->value.integer.value[0];
	...
	aw882xx->aw_pa->ops.aw_set_volume(aw882xx->aw_pa, value);
	aw882xx->aw_pa->volume_desc.init_volume = value;
...
static int aw882xx_dynamic_create_volume_control(struct aw882xx *aw882xx)
{
	struct snd_kcontrol_new *vol_control = NULL;
	char temp_buf[64] = { 0 };
	struct aw_volume_desc * vol_desc = &aw882xx->aw_pa->volume_desc;

	vol_control = devm_kzalloc(aw882xx->codec->dev, sizeof(struct snd_kcontrol_new),
				GFP_KERNEL);
	if (vol_control == NULL) {
		aw_dev_err(aw882xx->dev, "devm_kzalloc faild");
		return -ENOMEM;
	}

	aw882xx_mixer.reg = vol_desc->reg;
	aw882xx_mixer.shift = vol_desc->shift;
	aw882xx_mixer.min = (-vol_desc->mute_volume);
	aw882xx_mixer.max = vol_desc->max_volume;
	aw882xx_volume.private_value = (unsigned long)&aw882xx_mixer;

	memcpy(vol_control, &aw882xx_volume, sizeof(struct snd_kcontrol_new));

	snprintf(temp_buf, sizeof(temp_buf),
		"aw_dev_%d_rx_volume", aw882xx->index);
	vol_control->name = aw882xx_devm_kstrdup(aw882xx->dev, temp_buf);
	if (vol_control->name == NULL)
		return -ENOMEM;
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	aw882xx_dev_control[2].info = aw882xx_monitor_info;
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_force_boost_8v_mode", aw882xx->index);

	aw882xx_dev_control[3].name = kctl_name;
	aw882xx_dev_control[3].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw882xx_dev_control[3].info = aw882xx_force_boost_8v_mode_info;
	aw882xx_dev_control[3].get = aw882xx_force_boost_8v_mode_get;
	aw882xx_dev_control[3].put = aw882xx_force_boost_8v_mode_set;

	aw_componet_codec_ops.add_codec_controls(aw882xx->codec,
						aw882xx_dev_control, 4);

	ret = aw882xx_dynamic_create_volume_control(aw882xx);
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "create volume control failed!");
		return ret;
	}
...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	if (g_awinic_cfg == NULL) {
		...
		aw_cfg->len = (unsigned int)cont->size;
					...
					struct class_attribute *attr, const char *buf, size_t len)
{
	...
	if (value) {
		list_for_each(pos, dev_list) {
			...
			mutex_lock(&aw882xx->lock);
			aw882xx_hw_reset(aw882xx);
			aw882xx_dev_soft_reset(local_dev);
			aw882xx_dev_init_check(local_dev);
			aw882xx->aw_pa->reinit_flag = true;
			mutex_unlock(&aw882xx->lock);
		}
	}
	return len;
...
static struct class_attribute class_reinit_node =  \
		__ATTR(reinit, S_IWUSR | S_IRUGO,	\
		NULL, aw882xx_class_reinit_store);
...
static int aw882xx_reinit_class_init(struct aw882xx* aw882xx)
{
	...
	if (aw882xx->aw_pa->index != 0) {
		aw_dev_info(aw882xx->dev, "class node already register");
		return 0;
	}

	ret = class_register(&aw882xx_class);
	if (ret < 0) {
		aw_dev_info(aw882xx->dev, "error creating reinit class node");
		return ret;
	}


	ret = class_create_file(&aw882xx_class, &class_reinit_node);
	if (ret)
		aw_dev_info(aw882xx->dev, "creat class reinit fail");
				...
				const struct i2c_device_id *id)
{
	...
	/*create reinit class*/
	ret = aw882xx_reinit_class_init(aw882xx);
	if (ret)
		return ret;
